...
class ChanceGenerator {
  ...
  constructor(element, w, h){
    ...
    this.blocks = [
      ...
      /*3*/ x => Math.min(0.9,
        Math.max(0.3, Math.sin(x*0.6 + 0.7) )
      ),
      /*4*/ x => Math.max( Math.min(x*0.15-0.05, 0.6) , 0 ),
      /*5*/ x => Math.min( 0.8, Math.max( x*0.16+0.33, 0.5) - 0.5 ),
      /*6*/ x => Math.max(x*0.13, 0.5) - 0.5, //placeholder, below too
      /*7*/ x => Math.max(x*0.13, 0.5) - 0.5,
      /*8*/ x => Math.max(x*0.13, 0.5) - 0.5,
      /*9*/ x => Math.max(x*0.13, 0.5) - 0.5,
      /*10*/ x => Math.max(x*0.13, 0.5) - 0.5,
    ];
  ...
  coin(){
    let range = 0;
    let chances = [0]; // per block value, contains heighest chance index
    let lvl = window.game.level;
    let y = 0;
    let coin = 0;
...
window.ChanceGenerator = ChanceGenerator;
